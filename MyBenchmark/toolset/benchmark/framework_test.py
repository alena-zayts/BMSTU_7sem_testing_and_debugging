...
class FrameworkTest:
    ...
    def is_accepting_requests(self):
        ...
        url = "http://%s:%s%s" % (self.benchmarker.config.server_host, self.port,
                                  self.runTests[test_type].get_url())

        return self.benchmarker.docker_helper.test_client_connection(url)

    # Verifies each of the URLs for this test (simply curl the URL and check for it's return status.)
    def verify_urls(self):
        ...
        def verify_type(test_type):
            ...
            with open(os.path.join(verificationPath, 'verification.txt'), 'w') as verification:
                ...
                base_url = "http://%s:%s" % (self.benchmarker.config.server_host, self.port)
                ...
                try:
                    ...
                    if not any(result == 'fail' for (result, reason, url) in results):
                        self.benchmarker.docker_helper.test_client_connection(base_url + test.get_url())

                except ConnectionError as e:
                    results = [('fail', "Server did not respond to request", base_url)]
                    log("Verifying test %s for %s caused an exception: %s" % (test_type, self.name, e), color=Fore.RED)
                except Timeout as e:
                    results = [('fail', "Connection to server timed out", base_url)]
                    log("Verifying test %s for %s caused an exception: %s" % (test_type, self.name, e), color=Fore.RED)
                except Exception as e:
                    results = [('fail', "Rewritten exception issue: %s\n%s" % (e, traceback.format_exc()), base_url)]
                    log("Verifying test %s for %s caused an exception: %s" % (test_type, self.name, e), color=Fore.RED)
                ...
                def output_result(result, reason, url):
                    ...
                    log("   {!s}{!s}{!s} for {!s}".format(color, result.upper(), Style.RESET_ALL, url),
                        file=verification)

...
class DockerHelper:
    ...
    def __stop_all(docker_client):
        for container in docker_client.containers.list():
            if len(container.image.tags) > 0 \
                    and 'my_bm' in container.image.tags[0] \
    ...
    def start_database(self, database):
        ...
        container.
        '''
        image_name = "my_bm/%s:latest" % database
        log_prefix = image_name + ": "
    ...
    def build_wrk(self):
        '''
        Builds the my_bm/bm.wrk container
        '''
        self.__build(
            base_url=self.benchmarker.config.client_docker_host,

...
Запустите скрипт run_unit_tests.bat (Windows) или run_unit_tests.bat (Linux), который лежит в корне склонированного репозитория. 
...
Если На Windows: что-то идет не так, то скрипт лучше запустить вот так: cmd /k cmd /c run_unit_tests.bat (так хоть скрипт не закроется)
